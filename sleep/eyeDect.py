...
def drowsy(filename):
  ...
  print('저장되는 디렉토리', save_dir)
  ROOT = Path(os.path.relpath(ROOT, Path.cwd()))  # relative
  filepath = ROOT / settings.MEDIA_ROOT / filename
  print('머라고 뜨지? 이건 뭐지', filepath) 
...
# gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY) #색변환? 왜하는거지?
  ...
  for face in faces:
   ...
   #양눈의 종횡비 합/2
    ...
    EAR = round(EAR,2)
    print("EAR 값이 몇?", EAR)
    if EAR < 0.50:#종횡비가 0.5미만일때 문구출력(눈작은 사람이면 계속 경고)
    #cv2.putText(frame,"DROWSY",(20,100),
      img2 = cv2.putText(img, "DROWSY",(20,100), cv2.FONT_HERSHEY_SIMPLEX,3,(0,0,255),4)
